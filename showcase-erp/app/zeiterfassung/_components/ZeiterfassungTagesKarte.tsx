...
import { MdAccessTime, MdPlayArrow, MdStop, MdCoffee } from "react-icons/md";
import {
  ZeitbuchungEntry,
  formatTime,
  formatDateDE,
...
export default function ZeiterfassungTagesKarte({ entry, dateLabel }: Props) {
  if (!entry) {
    return (
  ...
  const diff = netto - SOLL_STUNDEN_PRO_SCHICHT;
    ...
    <div className="card bg-base-100 border border-base-300 shadow-sm">
      <div className="card-body p-4 sm:p-6">
        <h3 className="card-title text-sm font-medium text-base-content/70">
          ...
          {dateLabel} · {getWeekdayShort(entry.datum)}{" "}
