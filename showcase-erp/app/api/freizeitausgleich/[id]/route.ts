...
export async function PATCH(
  ...
  const antrag = await prisma.freizeitausgleichAntrag.findUnique({
    where: { id },
  });
  if (!antrag) {
    return NextResponse.json(
      { error: "Freizeitausgleich-Antrag nicht gefunden" },
      { status: 404 },
    );
  }
  ...
  if (status === "GENEHMIGT") {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.freizeitausgleichAntrag.update({
        where: { id },
      ...
      for (const dateStr of dates) {
        ...
        if (existing) {
          ...
          // Mark as FZA with originalSchicht="X_FREI" so the UI can
          // distinguish compensatory time off from a regular day off.
    ...
    await prisma.freizeitausgleichAntrag.update({
      where: { id },
      data: { status: "ABGELEHNT" },
    });
  }

  const updated = await prisma.freizeitausgleichAntrag.findUnique({
    where: { id },
  ...
  if (antrag.status === "GENEHMIGT") {
    ...
    await prisma.$transaction(async (tx) => {
      for (const datumUTC of dates) {
        ...
        if (!zuteilung || zuteilung.schicht !== "X_FREI") continue;

        if (zuteilung.originalSchicht && zuteilung.originalSchicht !== "X_FREI") {
          await tx.zuteilung.update({
      ...
      await tx.freizeitausgleichAntrag.delete({ where: { id } });
    });
  } else {
    await prisma.freizeitausgleichAntrag.delete({ where: { id } });
  }
